...
function Register() {
    ...
    let validationSchema = yup.object().shape({
        name: yup.string().min(3, 'min length is 3').max(20, 'max length 10').required('name is required'),
        email: yup.string().email('not valid email').required('email is required'),
        password: yup.string().required('password is required').min(6, 'min length is 6'),
        rePassword: yup.string().oneOf([yup.ref('password')], 'not matched').required('password is required'),
        phone: yup.string().matches(/^01[0125][0-9]{8}$/, 'not valid').required('phone is required')
    })
    ...
    return <>
        <h2 className='text-4xl font-bold text-emerald-500 my-7 text-center'>Register Now</h2>
        <form onSubmit={formik.handleSubmit} className="max-w-md mx-auto">
            <div className="relative z-0 w-full mb-5 group">
                ...
                <label htmlFor="name" className="peer-focus:font-medium absolute text-sm text-gray-500  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-emerald-600 peer-focus:dark:text-emerald-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Name </label>
            ...
            <div className="relative z-0 w-full mb-5 group">
                ...
                <label htmlFor="rePassword" className="peer-focus:font-medium absolute text-sm text-gray-500  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-emerald-600 peer-focus:dark:text-emerald-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">rePassword </label>
            ...
            <p className='mt-2'>Have acount? <Link className='font-semibold' to='/login'>Login</Link></p>
